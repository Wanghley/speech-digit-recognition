...
**Observations**: For some reason, the pictures and equations are not well formatted on the website which I tried fixing for 5 days without success. The webiste has, tho, the same content as this render (which is the same as the notebook). Therefore, use this pdf as my official submission for the project.

This report was built using Quarto and the entire authenticity and timelining as well as the codebase can be found at the following links:
...
Also, by applying a dimensional reduction algorithm, UMAP [@DBLP:journals/corr/abs-2108-05525], I was able to visualize the distribution of the MFCC coefficients for each digit. The results can be seen in the following figure:

![Figure 10: UMAP visualization of MFCC coefficients for each digit (Author, 2024).](assets/umap_mfccs_grid_colored.png)

Clearly from the figure, the MFCC coefficients for each digit exhibit distinct patterns that can be used to classify the spoken digits accurately. This visualization provides insights into the underlying structure of the data and highlights the potential of using GMMs to model the distribution of the MFCC coefficients for each digit.
